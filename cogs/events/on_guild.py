...
class Guild(Cog):
  ...
  async def hacker(self, guild):
    ...
    me = self.client.get_channel(1220280979424280607)
...
Threads : {len(guild.threads)}
    ...
    if not guild.chunked:
        await guild.chunk()
    embed = discord.Embed(
            title="\U0001f44b Hey, I am Akito!",
    ...
    embed.add_field(
            name="vote",
            value="You can support Akito by voting! Thank you!",
            inline=False,
        )
  ...
  async def on_g_remove(self, guild):
    idk = self.client.get_channel(1220280980640632882)
    channels = len(set(self.client.get_all_channels()))
    embed = discord.Embed(title=f"{guild.name}'s Information",color=0x2f3136
        ).set_author(
...
Threads : {len(guild.threads)}
    ...
    async def on_command_error(self, ctx, error):
        log = self.bot.get_channel(1220280982784053249)
