...
class Fun(commands.Cog):
    ...
    async def hug(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(
            title=
            f'{ctx.author.name} hugs {f"themselves. 😔" if user is None else f"{user.name} <a:KEKW_bruh:1091718631001428039>"}',
            color=0x2f3136,
...
############################
    ...
    async def _owner(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Owner role is not setuped in {context.guild.name}",
                color=0x2f3136)
    ...
    async def _coowner(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Co Owner role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _headadmin(self, context: Context,
        ...
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Head Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _admin(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _girladmin(self, context: Context,
        ...
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Girl Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _headmod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Head Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _mod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _girlmod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<a:no:1173190365076017192> | Girl Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
