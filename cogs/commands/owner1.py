...
class PaginationViewWallah:
  ...
  async def start(self, ctx, disxd=False):
    ...
    menu.add_button(bax)
    bax2 = ViewButton(style=discord.ButtonStyle.danger, label='-',emoji=None, custom_id=ViewButton.ID_END_SESSION)
...
class owner1(Cog):
  ...
  def __init__(self, client: Astroz):
    self.client = client
    self.owner_ids = [1140999483770023977]
  ...
  async def np_add(self, ctx: Context, user: discord.User):
    if ctx.message.author.id not in self.owner_ids:
            await ctx.send("<a:no:1173190365076017192> | this command can only be executed by my developers .")
            return
    with open('info.json', 'r') as idk:
      data = json.load(idk)
    np = data["np"]
    if user.id in np:
      await ctx.send(embed=discord.Embed(description=f"<a:no:1173190365076017192> | {user.name} Already have no prefix",color=0x0d0d13))
    else:
      # ... existing code
      data["np"].append(user.id)
      with open('info.json', 'w') as idk:
        json.dump(data, idk, indent=4)
      await ctx.send(embed=discord.Embed(description=f"<a:cx_tick:1173190431295676416> | {user.name} has been added to no prefix list", color=0x0d0d13))
  ...
  async def np_remove(self, ctx: Context, user: discord.User):
    ...
    if user.id not in np:
      await ctx.send(embed=discord.Embed(description=f"<a:no:1173190365076017192> | {user.name} Doesn't have no prefix",color=0x0d0d13))
    else:
      data["np"].remove(user.id)
    with open('info.json', 'w') as idk:
      json.dump(data, idk, indent=4)
      await ctx.send(embed=discord.Embed(description=f"<a:cx_tick:1173190431295676416> | Removed no prefix from {user.name}",color=0x0d0d13))
  ...
  async def bye_buddy(self, ctx):
    ...
    with open("info.json", "r") as f:
      file = json.load(f)
    if file["np"] == []:
      return await ctx.send("<a:no:1173190365076017192> | No one have no prefix.")
    comrade = []
    for idk in file["np"]:
