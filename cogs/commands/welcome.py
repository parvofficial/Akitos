...
```""",
    ...
    async def welctestt(self, ctx):
        ...
        if chan == []:
            ...
            if emautodel == 0 or emautodel == "":
                ...
                emautodel = emautodel
            em = discord.Embed(description=msg, color=0x0d0d13)
    ...
    async def _config(self, ctx):
        ...
        if chan == []:
            ...
            embed.add_field(name="**Welcome Footer:**", value=f"{emfooter}")
    ...
    async def _reset(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["welcome"]["channel"] == []:
                embed = discord.Embed(
                    description=
                    "<a:no:1173190365076017192> | This server don't have any greet channel setuped yet .",
                    color=0x0d0d13)
                await ctx.send(embed=embed)
            else:
                ...
                hacker = discord.Embed(
                    description=
                    "<a:cx_tick:1173190431295676416> | Succesfully cleared all greet config for this server .",
                    color=0x0d0d13)
                await ctx.send(embed=hacker)
        else:
    ...
    async def set_greet_footer(self, ctx, *, footer_message):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            hacker = discord.Embed(
                color=0x0d0d13,
                description=
                f"<a:cx_tick:1173190431295676416> | Successfully updated the greet footer message.",
                timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
