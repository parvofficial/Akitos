...
class Moderation(commands.Cog):
  ...
  async def unlockall(self, ctx, server: discord.Guild = None, *, reason=None):
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      "<a:cx_tick:1173190431295676416> | Unlocking all channels in few seconds .",
  ...
  async def lockall(self, ctx, server: discord.Guild = None, *, reason=None):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      "<a:cx_tick:1173190431295676416> | Locking all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    await ctx.reply(embed=hacker)
  ...
  async def hackban(self, ctx, userid, *, reason=None):
    ...
    try:
      ...
      await ctx.reply(embed=discord.Embed(
        description=
        f"<a:cx_tick:1173190431295676416> | Successfully hackbanned {userid} for {reason}",
        color=0x00FFCA))
    except:
      await ctx.reply(embed=discord.Embed(
  ...
  async def give(
    ...
    if guild.me.top_role >= ctx.author.top_role:
      embed = discord.Embed(
        color=0x00FFCA,
        description=
        "<a:no:1173190365076017192> | Your top role should be above my top role."
      )
      await ctx.send(embed=embed)
      return
    if role.position >= guild.me.top_role.position:
      await ctx.send(
        f"**<a:no:1173190365076017192> | The position of `{role}` is above my top role. **"
      )
      return
    else:
  ...
  async def _hideall(self, ctx):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      "<a:cx_tick:1173190431295676416> | Hiding all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def _unhideall(self, ctx):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | Unhiding all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    #hacker.set_thumbnail(url =f"{ctx.author.avatar}")
  ...
  async def _hide(self, ctx, channel: discord.TextChannel = None):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | Succefully Hidden {channel.mention} .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
  ...
  async def _unhide(self, ctx, channel: discord.TextChannel = None):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | Succefully Unhidden {channel.mention} .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
...
Reason: `{entry.reason}`\n\n''')
  ...
  async def _prefix(self, ctx: commands.Context, prefix):
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      ...
      await ctx.reply(embed=discord.Embed(
        description=
        f"<a:cx_tick:1173190431295676416> | Successfully Changed Prefix For **{ctx.guild.name}**\nNew Prefix for **{ctx.guild.name}** is : `{prefix}`\nUse `{prefix}help` For More info .",
        color=0x00FFCA))
    else:
  ...
  async def _softban(self,
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | Sucessfully soft-banned {member}.",
      timestamp=ctx.message.created_at)
...
# mute cmd is in mod.py
    ...
    if member.is_timed_out():
      try:
        ...
        hacker5 = discord.Embed(
          color=0x00FFCA,
          description=
          f"<a:cx_tick:1173190431295676416> | Successfully Unmuted {member.name}",
          timestamp=ctx.message.created_at)
        ...
        hacker = discord.Embed(
          color=0x00FFCA,
          description=
          "<a:no:1173190365076017192> | Unable to Remove Timeout:\n```py\n{}```"
          .format(e),
          timestamp=ctx.message.created_at)
        await ctx.send(embed=hacker)
    else:
      hacker1 = discord.Embed(
        color=0x00FFCA,
        description="<a:no:1173190365076017192> | {} Is Not Muted".
  ...
  async def _kick(self,
    ...
    if ctx.author.top_role.position > member.top_role.position or member == ctx.guild.owner:
      ...
      hacker = discord.Embed(
        color=0x00FFCA,
        description=
        f"<a:no:1173190365076017192> | {member.display_name} has been kicked from this guild, for: {reason}",
        timestamp=ctx.message.created_at)
      #hacker.set_footer(text=f"Made With 💖 By ~ GODFATHER ! </>#9417",icon_url= "https://media.discordapp.net/attachments/1066637418557624340/1068042323088379914/2491-couple-matching-1-2.gif")
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
    ...
    if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
      await ctx.send(
        "*<a:no:1173190365076017192> | You cannot kick someone with a higher role than you!*"
      )
  ...
  async def _warn(self,
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | {member.display_name} has been warned for: {reason}",
      timestamp=ctx.message.created_at)
  ...
  async def _ban(self,
    ...
    if ctx.author.top_role.position > member.top_role.position or ctx.author == ctx.guild.owner:
      ...
      hacker = discord.Embed(
        color=0x00FFE4,
        description=
        f"<a:cx_tick:1173190431295676416> | {member.display_name} has been successfully banned for the reason: `{reason}`",
        timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url=f"{ctx.author.avatar}")
      ...
      hacker1 = discord.Embed(
        color=0x00FFE4,
        description=
        f"❗ | You have been banned from {ctx.message.guild.name} for reason: `{reason}`",
        timestamp=ctx.message.created_at)
      await member.send(embed=hacker1)
    if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
      embed = discord.Embed(
        color=0x00FFE4,
        description=
        f"*<a:no:1173190365076017192> | You cannot ban someone with a higher role than you.*",
        timestamp=ctx.message.created_at)
      await ctx.send(embed=embed)
  ...
  async def _unban(self, ctx: commands.Context, id: int):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:cx_tick:1173190431295676416> | {user.name} has been successfully unbanned",
      timestamp=ctx.message.created_at)
    #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
    ...
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      f"<a:cx_tick:1173190431295676416> | {channel.name} has been successfully cloned",
      timestamp=ctx.message.created_at)
    #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member,
    ...
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      f"<a:cx_tick:1173190431295676416> | Successfully changed nickname of {member.name}",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
  ...
  async def _lock(self,
    ...
    try:
      ...
      await ctx.send(embed=discord.Embed(
        title="Akito | Lockdown",
        description=
        "<a:cx_tick:1173190431295676416> | Successfully locked **%s**" %
        (channel.mention),
        color=0x00FFCA))
    except:
      await ctx.send(
        embed=discord.Embed(title="Akito | Lockdown", 
  ...
  async def _unlock(self,
    ...
    try:
      await channel.set_permissions(
        ...
        reason=reason)
      await ctx.send(embed=discord.Embed(
        title="Akito | Unlockdown",
        description=
        "<a:cx_tick:1173190431295676416> | Successfully unlocked **%s**" %
        (channel.mention),
        color=0x00FFCA))
    except:
      await ctx.send(
        embed=discord.Embed(title="Akito | Unlockdown",
...
**Description:** {pack.description}
  ...
  async def kljl(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:cx_tick:1173190431295676416> | Removing roles from all humans")
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:cx_tick:1173190431295676416> | Removed mentioned role from all members'
    )
  ...
  async def kjjk(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:cx_tick:1173190431295676416> | Removing roles from all")
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:cx_tick:1173190431295676416> | Removed mentioned role from all')
  ...
  async def lkklkl(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:cx_tick:1173190431295676416> | Removing roles to all humans & bots"
    )
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:cx_tick:1173190431295676416> | Removed mentioned role from all bots'
    )
