...
class Utility(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.db = self.connection["Akito"]["servers"]
    ...
    async def invite(self, ctx: commands.Context): 
        embed = discord.Embed(
            title="** <a:999:1223904240867606608> Akito's Invite**<a:999:1223904240867606608> ",
            description=
            "> • **[Invite Me](https://discord.com/api/oauth2/authorize?client_id=1173097825614180463&permissions=8&scope=bot)\n> • [Support Server](https://discord.gg/)\n> • [Vote Me](https://top.gg/bot/)**",
            color=0x41eeee)     
        await ctx.send(embed=embed)
    ...
    async def _vote(self, ctx):
        button = Button(label="Vote", url = "https://top.gg/bot/")

        embed = discord.Embed(color=discord.Colour(0x0d0d13), description="**[Click Here](https://top.gg/bot/)** To Vote Me.")
        view = View()
        view.add_item(button)
        embed.set_author(name='Vote Akito',
...
""")
    ...
    async def _userinfo(self,
        ...
        if member.public_flags.hypesquad:
            badges += "<:hypesquad_flag:1174244131309101056> "
        if member.public_flags.hypesquad_balance:
            badges += "<:Balance:1174244159222206506> "
        if member.public_flags.hypesquad_bravery:
            badges += "<:Bravery:1174244099168149584> "
        if member.public_flags.hypesquad_brilliance:
            badges += "<:Brilliance:1174244056818253855> "
        if member.public_flags.early_supporter:
            badges += "<a:earlysup:1174245209048088588> "
        if member.public_flags.active_developer:
            badges += "<:active_dev:1174245176127000626> "
        if member.public_flags.verified_bot_developer:
            badges += "<a:dev:1174245143906365440>> "
        if member.public_flags.discord_certified_moderator:
            badges += "<:discord_certified_moderator_flag:1174245114344906752> "
        if member.public_flags.staff:
            badges += "<:staff_flag:1174245085639086181>"
        if member.public_flags.partner:
            badges += "<:partner_flag:1174246017831555082> "
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<a:cx_tick:1173190431295676416> Yes' if member.bot else '<a:no:1173190365076017192> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '000000'}
    ...
    async def status(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(embed=discord.Embed(
            title="**<a:general:1173088780253679618> | status**",
    ...
    async def emoji(self, ctx, emoji: discord.Emoji):
        return await ctx.send(embed=discord.Embed(
            title="**<a:general:1173088780253679618> | emoji**",
            description="emoji: %s\nid: **`%s`**" % (emoji, emoji.id),
            color=0x41eeee))
    ...
    async def removeemoji(self, ctx, emoji: discord.Emoji):
        await emoji.delete()
        await ctx.send(
            "**<a:cx_tick:1173190431295676416> emoji has been deleted.**")
    ...
    async def joined_at(self, ctx):
        ...
        embed = discord.Embed(title="Member Joined at", color=0x41eeee)
    ...
    async def github(self, ctx, *, search_query):
        ...
        if json["total_count"] == 0:
            embed = discord.Embed(description="No matching repositories found", color=0xFF5733)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title=f"First result for '{search_query}':", description=json['items'][0]['html_url'], color=0x7289DA)
            await ctx.send(embed=embed)
    ...
    async def _badges(self, ctx, user: Optional[discord.User] = None):
        ...
        if bdgs == []:
            ...
            embed2.add_field(
                name="**Bot Badges:**",
                value="**<a:Cosy_girl_shy:1223892585010036838> Family**",
                inline=False)
            ...
            embed.add_field(name="**User Badges:**",
                            value=f"{badges}",
                            inline=False)
        if len("\n".join([bdg for bdg in bdgs])) > 1024:
            embed.add_field(name="**Bot Badges:**",
                            value="\n".join([bdg for bdg in bdgs[:10]]),
                            inline=False)
        else:
            embed.add_field(name="**Bot Badges:**",
    ...
    async def stats(self, ctx):
        """Shows some usefull information about Akito"""
        loading_message = await ctx.send(embed=discord.Embed(
        color=0x2f3136,
        description="**<a:loading:1223892618245832796> Fetching Info From Database...**"
    ))
        ...
        gulzaib = await self.bot.fetch_user(1007689908753809439)
        #button2 = Button(label="Vote", url = "https://top.gg/bot/1136504108937908305/vote")
        button = Button(label="Support Server", url = "https://discord.gg/")
        button1 = Button(label="Invite Me", url = "https://discord.com/api/oauth2/authorize?client_id=1173097825614180463&permissions=8&scope=bot")
        ...
        embed = discord.Embed(
            color=0x2f3136,
            description=
            "**Hey, It's Me Akito A Feature Rich Advanced Multipurpose Bot. Build The Community Of Your Dreams With Me. Try Akito Now!**"
        )

        embed.add_field(
            name="**<a:moon:1172833540904337458> DEVELOPER**",
            value=f"[parv.op](https://discord.com/users/1140999483770023977)")
        embed.add_field(
            name="**<a:crown_1:1173206779354693735> OWNERS**",
            value=f"[parv.op](https://discord.com/users/1140999483770023977)",
        inline=False)
