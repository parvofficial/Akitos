...
Aliases""")
    ...
    async def membercount(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="<a:Mingle96:1222431510733520916> **MemberCount <a:Mingle96:1222431510733520916>**\n- %s members" % (len(ctx.guild.members)),
    )
    ...
    @commands.command(name="users", help="check users of Akito .")
    @blacklist_check()
    @ignore_check()
    async def users(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"**Users:**",
            description=
            f"**{len(set(self.bot.get_all_members()))} Users Of Akito**",
            color=0x2f3136)
        await ctx.send(embed=embed)
    ...
    async def pikachu(self, ctx: commands.Context):
        ...
        embed.set_footer(name="Made By parv.op", icon_url="")
    ...
    async def cleanup(self, ctx: commands.Context, amount: int):
        ...
        async for message in ctx.message.channel.history(limit=amount).filter(
            ...
            try:
                ...
                pass
        await msg.edit(content="cleaned up <a:cx_tick:1173190431295676416>")
