...
class OnOrOff(BasicView):
    ...
    @discord.ui.button(label="Yes", emoji="<:tick:1105683063524044811>", custom_id='Yes', style=discord.ButtonStyle.green)
    async def dare(self, interaction, button):
        self.value = 'Yes'
        self.stop()

    @discord.ui.button(label="No", emoji="<:cross1:1105683592375455754>", custom_id='No', style=discord.ButtonStyle.danger)
...
class afk(commands.Cog):
    ...
    async def on_message(self, message):
      try:
        ...
        if message.mentions:
            for user_mention in message.mentions:
                if afk[f'{user_mention.id}']['AFK'] == 'True':
                    ...
                    wl = discord.Embed(description=f'**{str(user_mention)}** went AFK <t:{ok}:R> : {reason}', color=0x00FFED)
                ...
                if afk[f'{message.author.id}']['AFK'] == 'True':
                    ...
                    wlba = discord.Embed(description=f'Welcome Back {str(message.author.mention)}, You got **{mentionz}** Mentions while You were AFK, I removed your AFK. You were afk for **{been_afk_for}**', color=0x00FFED)
                    ...
                    if afk[f'{user_mention.id}']['AFK'] == 'True':
                        ...
                        reason = afk[f'{user_mention.id}']['reason']
                        ok = afk[f'{user_mention.id}']['time']
                        wlt = discord.Embed(description=f'**{str(user_mention)}** went AFK <t:{ok}:R> : {reason}', color=0x00FFED)
        ...
        if afk[f'{message.author.id}']['AFK'] == 'True':
            ...
            afk[f'{message.author.id}']['guild'].remove(message.guild.id)
            afk[f'{message.author.id}']['reason'] == None
            wlbat = discord.Embed(description=f'Welcome Back {str(message.author.mention)}, You got **{mentionz}** Mentions while You were AFK, I removed your AFK. You were afk for **{been_afk_for}**', color=0x00FFED)
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if 'discord.gg' in reason:
            emd = discord.Embed(description=f"You can't Advertise in Afk Reason", color=0x00FFED)
        ...
        if '.gg/' in reason:
            eme = discord.Embed(description=f"You can't Advertise in Afk Reason", color=0x00FFED)
            return await ctx.send(embed=eme)

        view = OnOrOff(ctx)
        

        em = discord.Embed(description="Shall the bot DM you on every mention while you're afk?", color=0x00FFED)
        try:
        ...
        await test.delete()
        af = discord.Embed(description=f'{ctx.author.mention} Your AFK is now set to: {reason}', color=0x00FFED)
