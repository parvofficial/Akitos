...
class Info(commands.Cog):
  ...
  async def serverinfo(self, ctx: commands.Context):
        ...
        if guild.system_channel_flags.join_notifications:
          haa = "**System Welcome Messages**: <a:cx_tick:1173190431295676416>"
        else:
          haa = "**System Welcome Messages**: <a:no:1173190365076017192>"
        afkk = ctx.guild.afk_channel
        if ctx.guild.system_channel_flags.premium_subscriptions:
          skib = "**System Boost Messages**: <a:cx_tick:1173190431295676416>"
        else:
          skib = "**System Boost Messages**: <a:no:1173190365076017192>"  
        ...
        if ctx.guild.mfa_level == discord.MFALevel.disabled:
          mfa = "Enabled <a:cx_tick:1173190431295676416>"
        else:
          mfa = "Disabled <a:no:1173190365076017192>"
        embed.add_field(
            name="**__Extras__**",
            value=f"""
...
**2FA Requirement:** {mfa}
**Boost Bar Enabled:** {'<a:no:1173190365076017192>' if not ctx.guild.premium_progress_bar_enabled else '<a:cx_tick:1158669360223748106>'}
            """,
            inline=False
        )
        embed.add_field(
...
{guild.description}
        ...
        if guild.features:
            ...
            embed.add_field(
                name="**__Features__**",
                value='\n'.join([f"<a:cx_tick:1173190431295676416> : {feature}" for feature in dk]),
                inline=False
            )
...
Channels: <a:Hashtag:1223961311084548260> {len(guild.text_channels)} | <a:voicegoogle:1173088555229270130> {len(guild.voice_channels)} | <:stage:1223961969372037150>  {len(guild.stage_channels)} 
{bye}
            """,
            inline=False
        )
...
Total Emoji: {len(guild.emojis)}/{uf}
        ...
        embed.add_field(
            ...
            value=f"""
Level: {guild.premium_tier} <a:boost:1174243487256948867> {guild.premium_subscription_count} boosts] 
{byecom}
            """,
            inline=False
        )
