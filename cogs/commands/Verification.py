...
class Verification(commands.Cog):
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<a:Black_Verification:1194495008405999626>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In *{ctx.guild.name}* Press the <a:Black_Verification:1194495008405999626> Button Below", color=0x2f3136)
