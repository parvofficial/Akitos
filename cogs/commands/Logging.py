...
class Logging(commands.Cog):
  ...
  @log.command(name="msglog", aliases=["messagelog"])
  async def _msglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("msg",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Message log channel updated to {channel.mention}")
    
    
  @log.command(name="memberlog", aliases=["memlog"])
  async def __msglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("member",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Member log channel updated to {channel.mention}")


  @log.command(name="serverlog", aliases=["svlog"])
  async def ___msglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("server",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Server log channel updated to {channel.mention}")

  @log.command(name="channellog", aliases=["chlog"])
  async def ___msglog_(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("channel",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Channel log channel updated to {channel.mention}")


  @log.command(name="rolelog", aliases=["rl"])
  async def __msglog__(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("role",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Role log channel updated to {channel.mention}")


  @log.command(name="modlog", aliases=["moderatelog"])
  async def teccnobetamsglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1173190365076017192> | Your top role should be above my top role.")
    save("mod",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | Mod log channel updated to {channel.mention}")
...
Disables all the log in the server."""
    ...
    embed.set_footer(icon_url=ctx.bot.user.avatar, text="Thanks For Choosing Akito!") 
  ...
  async def logallenable(self,ctx,channel:discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1173190431295676416>  | All logs channel are updated to {channel.mention}")
  ...
  async def _dis(self,ctx):
    if not await self._check(ctx):
      return await ctx.send("<a:no:1173190365076017192> | Your top role should be above my top role.")
    with open("loggingbyauth.json", "r") as f:
      fl = json.load(f)
    data = fl.get(str(ctx.guild.id))
    if data == {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}:
      return await ctx.reply("<a:no:1173190365076017192> | No logging is enabled.")
    fl[str(ctx.guild.id)] = {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}
    with open("loggingbyauth.json", "w") as f:
      json.dump(fl,f,indent=4)
    await ctx.reply("<a:cx_tick:1173190431295676416>  | Disabled all the logs for this server.")
  ...
  async def on_raw_bulk_message_delete(self, payload):
    ...
    embed.set_author(icon_url=self.bot.user.avatar, name=f"{self.bot.user.name}")
  ...
  async def on_member_remove(self, member):
    ...
    is_kicked = kick is not None
    embed = self.em_g("A member is no longer in the server", f"{member} {member.id}\n👤Account created at {discord.utils.format_dt(member.created_at)}", "LEFT" if not is_kicked else "KICKED", member, member.avatar, discord.Color.red())
    embed.set_thumbnail(url=member.avatar)
    
    if is_kicked:
        embed.title = "A member got kicked from the server"
        embed.add_field(name="Kicked by:", value=str(mem))
        if reason:
  ...
  async def on_guild_channel_create(self, channel):
    async for logs in channel.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
      if logs.target.id == channel.id:
        ...
        category_name = "No Category" if not channel.category else f"{channel.category} (ID: {channel.category.id})"
