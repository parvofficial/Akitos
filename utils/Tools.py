...
async def check_voter(mem):
    async with aiohttp.ClientSession(
        ...
        {
            "Authorization":
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEwMTI2MjcwODgyMzIxNjUzNzYiLCJib3QiOnRydWUsImlhdCI6MTY3MDU4MzE3NH0.WULUKA8"
        }) as session:
        async with session.get(
                f"https://top.gg/api/bots/101262708/check?userId={str(mem)}"
        ) as response:
...
def is_voter():
    ...
    async def predicate(ctx: Context):
        async with aiohttp.ClientSession(
            ...
            {
                "Authorization":
                "eyJhbGciOiyJpZCI6IjEwMTI2MjcwODgyMzIxNjUzNzYiLCJib3QiOnRydWUsImlhdCI6MTY3MDU4MzE3NH0.WULUKASz45RZduUMpTCqzHt0nPk3MqnpeJHF3YNgBo8"
            }) as session:
            async with session.get(
                    f"https://top.gg/api/bots/10126270886/check?userId={str(ctx.author.id)}"
            ) as response:
                vote = await response.json()
    ...
    if str(guildID) not in data["guilds"]:
        defaultConfig = {
            "welcome": {
                ...
                "embed": False,
                "footer": "",
                "image": "https://media.discordapp.net/attachments/1131209916758114305/1131843467253927997/THANKS_FOR_JOIN_US__THANKS_FOR_YOUR_LOVING_SUPPORT.gif",
                "message": "**•** - Welcome to **<<server.name>>**.\n**•** - We hope you'll enjoy your stay here.\n**•** - Don't forget to checkout Akito bot <@1173097825614180463> .\n**•** - You Are **<<server.member_count>>**th Member Enjoy!!.",
                "ping": False,
                "title": "",
                "thumbnail": ""
            },
